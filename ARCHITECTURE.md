...
- Không có API check trạng thái riêng — gọi lại `command=charging` với thẻ đã xử lý chỉ trả lỗi "đã gửi trước", không phân biệt được đúng/sai. Nếu callback lỗi hạ tầng phía gachthefast (network/TLS cũ), dùng nút **Duyệt tay** ở `/admin/topups` (chỉ hiện cho `provider = CARD`, chặn cả UI lẫn server action).
